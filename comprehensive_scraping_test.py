...
class ComprehensiveScrapingTester:
    ...
    async def test_start_job(self):
        ...
        try:
            start_time = time.time()
            payload = {"priority": "normal", "custom_config": None}
            async with self.session.put(f"{self.base_url}/scraping/jobs/{job_id}/start", json=payload) as response:
                response_time = time.time() - start_time
    ...
    async def test_stop_job(self):
        ...
        try:
            start_time = time.time()
            async with self.session.put(f"{self.base_url}/scraping/jobs/{job_id}/stop") as response:
                response_time = time.time() - start_time
    ...
    async def test_pause_job(self):
        ...
        try:
            start_time = time.time()
            async with self.session.put(f"{self.base_url}/scraping/jobs/{job_id}/pause") as response:
                response_time = time.time() - start_time
    ...
    try:
        with open('/app/frontend/.env', 'r') as f:
            for line in f:
                ...
                base_url = "https://implementation-check-1.preview.emergentagent.com/api"
    except:
        base_url = "https://implementation-check-1.preview.emergentagent.com/api"
